...
export default {
  ...
  methods: {
    ...
    changeActive(spuSaleAttrValue, arr) {
      // console.log(spuSaleAttrValue,arr);
      // 遍历所有的售卖属性值isChecked ，设为 0 全部不高亮
      arr.forEach((item) => {
        item.isChecked = 0;
      });
    ...
    changSkuNum(e) {
      ...
      if (isNaN(val) ||  val < 1) {
        ...
        this.skuNum = parseInt(val); // 必须是整数，且大于1
      }
    ...
    async addShopCar() {
      ...
      // 路由跳转  路由需要传递信息，购买的数量
      // 数据需要传递给下一级路由
      // 数据存储可以通过会话存储(不持久化，会话结束数据再消失)
      // location和session 即本地存储和会话存储，一般是json字符串
